    ...
    return (
      <main className="min-h-[70vh] bg-[#fafaf9] px-6 py-20 lg:px-12">
        <div className="mx-auto flex max-w-3xl flex-col items-center justify-center text-center">
          ...
          <h1 className="text-3xl font-light text-[#1c1917]">{t.empty}</h1>
  ...
  return (
    <main className="bg-[#fafaf9] px-6 py-10 lg:px-12 lg:py-14">
      <div className="mx-auto max-w-[1400px]">
        <h1 className="mb-8 text-3xl font-light text-[#1c1917] lg:text-4xl">{t.title}</h1>

        <div className="grid gap-8 lg:grid-cols-[1fr_360px]">
          <section className="rounded-xl border border-gray-200 bg-white p-5 lg:p-6">
            <div className="mb-4 flex items-center justify-between">
              <p className="text-sm text-gray-500">{items.length} {t.count}</p>
              <button type="button" onClick={golesteCosul}
          ...
          <aside className="h-fit rounded-xl border border-gray-200 bg-white p-5 lg:sticky lg:top-28">
            <h2 className="text-lg font-semibold text-[#1a1a1a]">{t.orderSummary}</h2>
            ...
            <div className="mt-5">
              <p className="mb-2 text-xs uppercase tracking-wide text-gray-500">{t.payment}</p>
              <div className="flex items-center gap-2">
                {["Visa", "Mastercard", "PayPal"].map((method) => (
                  <div key={method} className="rounded border border-gray-200 bg-gray-50 px-2.5 py-1 text-xs text-gray-600">
                    {method}
                  </div>
                ))}
              </div>
